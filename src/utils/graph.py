...
class Graph:
    ...
    def style_edge(self, from_vertex: str, to_vertex: str, **kwargs) -> None:
        """
        Раскраска ребер

        Например: style_edge('S1', 'S2', edge_color='red', width=5)
        ...
        labels = nx.get_edge_attributes(self._G, 'label')
        if (from_vertex, to_vertex) in labels:
            labels[(from_vertex, to_vertex)] += f", {kwargs['label']}"
        self._G.add_edge(from_vertex, to_vertex, **kwargs)
        nx.set_edge_attributes(self._G, labels, 'label')

    def canvas(self, width: int = None, height: int = None) -> Figure:
        """


        :param width: в дюймах
        :param height: в дюймах
        :param dpi:
        :return:
        """
